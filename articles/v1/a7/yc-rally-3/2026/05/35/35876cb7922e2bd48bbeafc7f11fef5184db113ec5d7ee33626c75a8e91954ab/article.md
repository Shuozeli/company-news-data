---
schema_version: "1.0.0"
document_id: "35876cb7922e2bd48bbeafc7f11fef5184db113ec5d7ee33626c75a8e91954ab"
company_key: "yc-rally-3"
company: "Rally"
source_id: "yc-rally-3-news-import-75685e7d2a3f"
canonical_url: "https://www.getrally.com/blog/international-fuel-card-for-european-fleets"
published_at: "2026-05-11T00:00:00+00:00"
first_seen_at: "2026-07-22T10:49:14.710560+00:00"
fetched_at: "2026-07-28T22:00:13.771809+00:00"
content_hash: "sha256:125be30e8bb6507d3a666bfa912c898b033377c28c4d87b1993679b0bbb0bce4"
---

# International Fuel Cards for European Fleets in 2026 Compared

An international fuel card is no longer a niche product for long-haul hauliers. As more vans, service vehicles and electric fleets cross borders inside the EU, fleet managers and finance leads run into the same wall: domestic fuel cards stop working at the first foreign forecourt, VAT recovery is a paper-driven mess, and EV charging adds another layer of fragmentation. This guide is a neutral 2026 comparison of the international fuel cards European fleets shortlist most often.


If you are also evaluating UK-only providers, see our[UK fuel-card comparison](https://www.getrally.com/blog/best-fuel-card-uk-2026) . If you run a smaller team, our companion guide to[small-business fuel cards](https://www.getrally.com/blog/best-fuel-card-for-small-business) is the better starting point.


## What an International Fuel Card Actually Means


An international fuel card is a fleet payment card that works across multiple countries rather than a single domestic network. Three things distinguish it from a domestic card.


First, **cross-border acceptance** . The card is accepted at thousands of stations across the EU, the UK and often North Africa or Eastern Europe. Acceptance may run through a partner network (DKV, UTA, Eurowag) or a payment network like Visa (Rally). Either way, drivers refuel in five countries on one card without rejected transactions.


Second, **consolidated multi-currency invoicing** . Instead of a separate invoice per country, finance receives a single statement itemised by country, currency, supplier and VAT line, ready for accounting software or a VAT-reclaim agent.


Third, **integration of ancillary services** : motorway tolls, vignettes, truck-tolls (LKW-Maut, HU-GO, GO Box), EV charging, parking and AdBlue. The deeper the integration, the fewer side accounts finance has to chase.


## Why Fleets Switch from a Domestic Fuel Card


Three operational pains push fleets to upgrade.


**The card is declined at the border.** A driver who has used the same domestic card for years hits a French autoroute station or a Spanish Cepsa forecourt where the card is not accepted, pays personally, and creates a manual expense claim that finance reconciles two weeks later.


**VAT recovery is impossible without help.** Fuel bought in another EU country is recoverable under the 13th Directive and the EU VAT-refund procedure, but only with clean paperwork. Receipts in three languages, three VAT rates and three tax-authority formats rarely meet the bar. Many fleets simply absorb 19 to 27 percent of foreign-fuel cost as a loss.


**Tolls and vignettes live outside the fuel invoice.** Drivers buy vignettes at petrol stations, pay tolls in cash or on personal cards, and add motorway charges through unfamiliar OBUs. Each one is a separate receipt, a separate VAT treatment and a separate place fraud can hide.


An international fuel card collapses all three problems onto one account, one card and one monthly statement.


## 6 International Fuel Cards Compared in 2026


Below is a neutral provider-by-provider review. Coverage, fees and feature sets change frequently — these notes reflect publicly available information as of mid-2026 and should be confirmed with each provider for current pricing.


### Rally


Rally is a Visa-backed[Rally business fuel card](https://www.getrally.com/fuel-cards) and expense platform built for European fleets. Because it uses the Visa network rather than a partner-station network, the same card works at virtually any fuel station, EV charger, toll booth or merchant that accepts Visa — typically over 99% of forecourts across the UK, EU and 30+ countries.


- **Countries covered.** UK, EU and 30+ countries; acceptance follows the Visa network.
- **Pricing model.** Pump price with no list-price markup. Simple per-card monthly fee. No FX markups, no service fees per litre, no minimum spend.
- **VAT recovery.** Each transaction is captured with VAT line, country and supplier. Consolidated invoices per legal entity and per country, ready for HMRC, AEAT, BZSt or a 13th Directive reclaim.
- **Toll and vignette coverage.** Tolls and parking on the same card wherever the operator accepts Visa. HGV truck-toll OBUs (LKW-Maut, Telepass for trucks) still need a separate device for some routes.
- **EV charging.** Rally Charge covers 500,000+ charge points across 20+ countries, with sessions startable via fob or WhatsApp.
- **Best for.** Mixed fleets of vans, service vehicles and light commercial vehicles that cross borders and want EV charging on the same account.


### DKV Mobility


DKV is one of the oldest multi-brand fuel cards in Europe and the default choice for long-haul HGV operations, with over 50,000 acceptance points across roughly 50 countries. For the trade-off vs. a Visa-backed option, see our[Rally vs DKV](https://www.getrally.com/dkv-fuel-card-alternative) comparison.


- **Countries covered.** Around 50 countries, strong on long-haul corridors across Germany, France, Italy, the Benelux and Eastern Europe.
- **Pricing model.** Partner pricing — list price at the pump with a service fee. Volume rebates for larger fleets; deposit or bank guarantee often required.
- **VAT recovery.** DKV Refund Service handles foreign-VAT reclaim; widely regarded as the most mature in the market.
- **Toll and vignette coverage.** DKV BOX EUROPE supports HGV tolls in most EU markets; vignettes can be charged to the card.
- **EV charging.** DKV +Charge is growing but thinner outside Germany and the Benelux.
- **Best for.** Long-haul HGV operators that value mature toll integration and back-office VAT reclaim.


### UTA Edenred


UTA, now part of Edenred, runs about 70,000 service points across more than 40 countries. For a direct comparison, see[Rally vs UTA](https://www.getrally.com/uta-fuel-card-alternative) .


- **Countries covered.** 40+ countries including all major EU markets and several CIS states.
- **Pricing model.** Partner pricing with service fees. Bespoke quotes for medium and large fleets.
- **VAT recovery.** Edenred VAT-recovery support is offered as an add-on service.
- **Toll and vignette coverage.** UTA OnE box and partner OBUs cover French TIS PL, Italian Telepass, Spanish Via-T, LKW-Maut, HU-GO, Belgium Viapass and more.
- **EV charging.** UTA eCharge integrates roaming across European charging networks.
- **Best for.** Mid-to-large HGV fleets that want one of the broadest service-point networks in Europe.


### Eurowag


Eurowag is the strongest fuel card for Central and Eastern European HGV operations and is increasingly active in Western Europe. The platform covers roughly 35 countries and is often bundled with Eurowag Telematics.


- **Countries covered.** 35+ countries, with deep coverage in Czech Republic, Slovakia, Hungary, Poland and Romania.
- **Pricing model.** Pump-price or partner-price options depending on country; service fees apply.
- **VAT recovery.** Eurowag Tax Refund handles foreign VAT reclaim across the EU.
- **Toll and vignette coverage.** EETS-compliant Eurowag toll services cover most EU truck-toll regimes through a single OBU.
- **EV charging.** Eurowag eMobility adds CPO roaming across European networks.
- **Best for.** HGV operators with significant CEE exposure who want telematics and tolls on one vendor.


### EDC TruckOne


EDC is a Spanish provider that owns a network of high-capacity HGV truck stops on the Iberian Peninsula plus European partners. TruckOne is the multi-brand card built on that base.


- **Countries covered.** Around 30 countries, strongest in Spain, Portugal and France.
- **Pricing model.** Discounted pump or partner pricing inside EDC stations; partner pricing elsewhere.
- **VAT recovery.** Foreign VAT reclaim available through EDC partners.
- **Toll and vignette coverage.** Spanish Via-T and Portuguese Via Verde integrated; broader EU tolls via partners.
- **EV charging.** Growing CPO roaming on the Iberian Peninsula.
- **Best for.** Fleets with heavy Iberian routes — Spain–France, Spain–Portugal, intra-Iberian distribution.


### Andamur ProEurope


Andamur runs large truck-friendly service areas in Spain and France and issues the Andamur ProEurope card in partnership with a European multi-brand network.


- **Countries covered.** Around 30 European countries via partner networks.
- **Pricing model.** Strong discounts inside Andamur's own stations; partner pricing elsewhere.
- **VAT recovery.** Foreign VAT reclaim offered alongside the card.
- **Toll and vignette coverage.** Spanish, French and Portuguese tolls integrated.
- **EV charging.** Limited compared with the larger providers.
- **Best for.** HGV fleets crossing Spain and France that want premium service areas plus EU-wide acceptance.


For a quick side-by-side view, here is the comparison table:


Provider Countries Pricing model VAT recovery Tolls & vignettes EV charging Best for


**Rally** UK + 30+ EU Pump price, flat per-card fee Built-in VAT-ready invoices Visa-accepted tolls + parking 500K+ chargers, 20+ countries Mixed vans, service vehicles, EV-curious fleets


**DKV Mobility** ~50 Partner pricing + service fee DKV Refund Service DKV BOX EUROPE DKV +Charge Long-haul HGV operations


**UTA Edenred** 40+ Partner pricing + service fee Edenred VAT services UTA OnE box UTA eCharge Mid/large HGV fleets


**Eurowag** ~35 Pump or partner pricing Eurowag Tax Refund EETS-compliant OBU Eurowag eMobility CEE-focused HGV fleets


**EDC TruckOne** ~30 EDC-discounted + partner Partner-led Via-T, Via Verde, partner EU Iberian roaming Iberian-route HGVs


**Andamur ProEurope** ~30 Andamur-discounted + partner Partner-led ES/FR/PT tolls Limited Spain-France HGV corridors


Brands not covered in depth here include Aral Card (BP) and DKV's Shell-equivalent products. For a Rally vs Aral view specifically, see our[Rally vs Aral](https://www.getrally.com/aral-fuel-card-alternative) comparison page.


## How Cross-Border VAT Recovery Actually Works


For most European fleets, VAT recovery is where international fuel cards earn their keep. This is a high-level overview, not legal advice — every country has its own rules and a tax advisor should validate the workflow before you rely on it.


When a business pays VAT on fuel in an EU member state where it is not established, the recovery route is the **EU VAT-refund procedure** (the 8th Directive) for businesses established elsewhere in the EU, and the **13th Directive procedure** for non-EU established businesses, such as UK fleets after Brexit. Both rely on the same building blocks: a valid VAT invoice with a clean breakdown by country and rate, and a structured submission through the home country's tax portal (HMRC, AEAT, BZSt).


Manual recovery from paper receipts is slow and expensive. It is not uncommon for fleets to spend €40 to €120 in admin to recover €100 in VAT, which pushes many finance teams to skip recovery altogether.


An international fuel card fixes the upstream problem. The issuer produces a single VAT-ready invoice per country with supplier VAT number, date, station, fuel type, volume, net, VAT rate and gross — everything a reclaim agent needs.


That structured data can be exported directly into your accounting system or handed to a reclaim agent like DKV Refund, Eurowag Tax Refund or Edenred VAT services. Reclaim agents typically charge 5 to 12 percent of recovered VAT, which is far cheaper than running the workflow internally. The result is an effective fuel cost reduction of several percentage points across the fleet — often more meaningful than the headline pence-per-litre discount.


## Tolls, Vignettes, and Mautboxes — One Card or Two?


European tolling is a patchwork the EU's EETS project is slowly unifying but has not yet replaced. The 2026 state of play:


- **France.** A-roads run on TIS PL for trucks and Bip&Go-style tags for light vehicles; light tolls can be paid card-by-card at the booth.
- **Italy.** Telepass dominates; multi-brand fuel cards integrate with Telepass for HGVs.
- **Spain.** Via-T tags are required for many motorways; EDC, Andamur, DKV and UTA support Via-T.
- **Portugal.** Via Verde, supported by most international fuel cards.
- **Germany.** LKW-Maut applies to qualifying goods vehicles and vehicle combinations and runs on Toll Collect OBUs or EETS-compliant alternatives. Our[2026 LKW-Maut refund and toll-management guide](https://www.getrally.com/germany-lkw-maut-refund-for-fleets) covers current thresholds, eligibility, applications, and reconciliation.
- **Switzerland and Austria.** Vignettes for light vehicles plus separate HGV regimes (LSVA in Switzerland, GO Box in Austria).
- **Slovenia, Hungary, Czechia, Slovakia.** Each has its own vignette or e-toll regime, handled via dedicated OBUs.


The practical rule: for pure HGV operations, a multi-brand card with a real EETS OBU (DKV BOX EUROPE, UTA OnE, Eurowag) is still the right answer. For vans, service vehicles and lighter mixed fleets, a Visa-backed card pays light-vehicle tolls directly at the booth, consolidates them with fuel and EV charging on one invoice, and avoids the cost of a dedicated OBU.


## EV Charging Across Borders


EV charging is where the international fuel-card market is changing fastest. The historical model — a closed-loop EV card from one operator (Allego, Ionity, Chargemap, Plugsurfing) — works in the home country but roams poorly across borders. Drivers end up carrying three or four cards and switching apps for each network.


The fuel-card incumbents have responded with roaming products: DKV +Charge, UTA eCharge, Eurowag eMobility. These are real and useful, but the partner network behind them is thinner than the fuel network and coverage drops outside the issuer's home market.


A modern multi-country[EV charging card for fleets](https://www.getrally.com/ev-charging) like Rally Charge takes a different path: it aggregates roaming partners under a single account, so drivers can start any compatible charger across 20+ countries from one card, fob or WhatsApp message. For mixed van and service-vehicle fleets that electrify faster than long-haul HGVs, this is increasingly the deciding feature. Long-haul operators with HGV-specific charging needs (megawatt charging, dedicated truck stops) will need to follow the truck-charging buildout closely over the next two to three years.


## Choosing the Right Card for Your Fleet


The right international fuel card is the one that maps to your routes, fleet composition, payment terms and admin tolerance. A simple scoring framework cuts through the marketing.


1. **Routes.** List your top 10 corridors and the stations drivers actually use. Score each provider on real-world acceptance along those corridors, not advertised country counts.
2. **Pricing.** Pump price + flat fee (Rally) is predictable. Partner pricing + service fee (DKV, UTA, Eurowag) can deliver volume rebates at scale but adds variability.
3. **VAT and admin.** If you drive in three or more EU countries, integrated VAT-recovery or VAT-ready invoicing is worth real money. Score providers on the invoice file, not the brochure.
4. **Tolls.** For HGV-heavy fleets, a dedicated EETS OBU is non-negotiable. For light vehicles, a Visa-backed card usually covers tolls without extra hardware.
5. **EV.** If more than 10 percent of your fleet is electric, treat EV-charging coverage as a hard requirement.
6. **Onboarding.** Deposits, credit checks and minimum-volume commitments are still common with traditional providers. Newer Visa-backed options skip them.


Be honest about when a closed-loop card is still the right answer. Deep, truck-only HGV operations with mature LKW-Maut needs and large foreign-VAT volumes are still well-served by DKV, UTA or Eurowag. For mixed van, service-vehicle and lighter cross-border fleets that also need EV charging and clean accounting data, a Visa-backed card like Rally usually pays for itself in admin time alone.


If you would like to see how Rally compares to your current international card on real routes and a real invoice,[book a demo](https://www.getrally.com/demo?source=blog_inline) and we will run the numbers for your fleet.
