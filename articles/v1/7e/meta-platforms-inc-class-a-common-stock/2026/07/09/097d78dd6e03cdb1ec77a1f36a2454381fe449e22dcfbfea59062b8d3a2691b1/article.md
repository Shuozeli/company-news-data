...
document_id: "097d78dd6e03cdb1ec77a1f36a2454381fe449e22dcfbfea59062b8d3a2691b1"
...
published_at: "2026-07-01T16:00:36+00:00"
first_seen_at: "2026-07-20T04:35:35.936675+00:00"
fetched_at: "2026-07-28T21:52:24.088997+00:00"
content_hash: "sha256:a2114400640d0ec3dda2fac400cb69cb859dd51168c39ce664beeadcc60d75a7"
---
...
- Unified metadata schema: We rewrote the metadata subsystem and collapsed the metadata spread across different layers into one unified and flat schema backed by[ZippyDB](https://engineering.fb.com/2021/08/06/core-infra/zippydb/) . This paves the way for O(1) lookup to resolve paths to storage addresses, which is a step-function improvement.
...
Figure 3 shows the new request flow for


getObject(“/bucket/path”)


. When the SDK on the client receives this API call, it now issues a


getReadPlan(“/bucket/path”)
...
- Distributed data cache: We leveraged the spare memory on the GPU hosts as a distributed data cache for frequently and concurrently accessed data. To achieve this, we reused components from


[Meta’s Owl subsystem](https://engineering.fb.com/2022/07/14/data-infrastructure/owl-distributing-content-at-meta-scale/) : We integrated the peers in the Owl subsystem directly into the BLOB-storage client SDK so that all data access goes through this data cache.
...
1. A researcher curates data from various sources, enriches them and persists them in BLOB storage.


2. The researcher picks a region where they want to run the job.


3. The researcher submits a data-ingestion job, which creates a snapshot of the training datasets onto the target region in a file format optimized for data loading from within the GPU host.


4. The researcher then waits for ingestion to finish; depending on the dataset size, that can take hours.


5. The researcher submits their training job and monitors their run.


6. The researcher analyzes outputs, tweaks datasets, and iterates again, starting with Step 3.
...
- Dataloader prefetch: Dataloaders prefetch the next batch of datasets into memory while processing the current batch. This prefetch will surface as a


*read* operation at the BLOB-storage SDK level.
...
- [Share on Facebook (Opens in new window) Facebook](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=facebook)
- [Share on Threads (Opens in new window) Threads](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=threads)
- [Share on WhatsApp (Opens in new window) WhatsApp](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=jetpack-whatsapp)
- [Share on LinkedIn (Opens in new window) LinkedIn](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=linkedin)
- [Share on Reddit (Opens in new window) Reddit](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=reddit)
- [Share on X (Opens in new window) X](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=x)
- [Share on Bluesky (Opens in new window) Bluesky](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=bluesky)
- [Share on Mastodon (Opens in new window) Mastodon](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=mastodon)
- [Share on Hacker News (Opens in new window) Hacker News](https://engineering.fb.com/2026/07/01/data-infrastructure/metas-ai-storage-blueprint-at-scale/?share=custom-1699562127)
