...
document_id: "f76f41ada2ea2b9c0224c9a2ff9ca891de92f94552ba34e3c3c247f2205c02bd"
company_key: "yc-unsloth-ai"
company: "Unsloth AI"
source_id: "yc-unsloth-ai-news-import-bb4a9dd43ab2"
canonical_url: "https://unsloth.ai/blog/qwq-32b"
published_at: "2025-03-07T00:00:00+00:00"
first_seen_at: "2026-07-24T05:34:23.482806+00:00"
fetched_at: "2026-07-28T21:30:42.971376+00:00"
content_hash: "sha256:d6e21b90bb34c82693b49e1ec95924547710bb68ccd3a59a328f6b6db0d913d1"
---
...
We found a few issues as well specifically impacting finetuning! The EOS token is correct, but the PAD token should probably rather be "<|vision_pad|>" We updated it in[here](https://huggingface.co/unsloth/QwQ-32B/blob/main/tokenizer_config.json) .


` "eos_token": "<|im_end|>", "pad_token": "<|endoftext|>",` ⚙️ Official Recommended Settings
...
` top_k=40 top_p=0.95 min_p=0.0 temperature=0.6 dry typ_p xtc`
...
You can read our complete guide in our[docs here](https://docs.unsloth.ai/basics/tutorial-how-to-run-qwq-32b-without-bugs#tutorial-how-to-run-qwq-32b) . Obtain the latest llama.cpp at:[github.com/ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp) . You can follow the build instructions below as well. Change -DGGML_CUDA=ON to -DGGML_CUDA=OFF if you don't have a GPU or just want CPU inference.
...
Download the model via (after installing pip install huggingface_hub hf_transfer ). You can choose Q4_K_M, or other quantized versions (like BF16 full precision). Other variants:[huggingface.co/unsloth/QwQ-32B-GGUF](https://huggingface.co/unsloth/QwQ-32B-GGUF)
Then run Unsloth's Flappy Bird test, which will save the output to Q4_K_M_yes_samplers.txt
...
See example final Python output[here](https://docs.unsloth.ai/basics/tutorial-how-to-run-qwq-32b-properly#example-final-python-output-removed-thinking-parts) . The full input is:
...
- QwQ's context length was not natively 128K, but rather 32K with YaRN extension. We tried[overriding llama.cpp](https://docs.unsloth.ai/basics/tutorial-how-to-run-qwq-32b-without-bugs#experimental-results-and-extra-notes) 's YaRN handling, but nothing changed. For example in the[QwQ-32B readme](https://huggingface.co/Qwen/QwQ-32B) file we see the below:
- ` { ..., "rope_scaling": { "factor": 4.0, "original_max_position_embeddings": 32768, "type": "yarn" } }`
- We also thought maybe the RMS Layernorm epsilon was wrong - not 1e-5 but maybe 1e-6. For example[this](https://huggingface.co/Qwen/Qwen2.5-32B-Instruct/blob/main/config.json) has rms_norm_eps=1e-06, whilst[this](https://huggingface.co/Qwen/Qwen2.5-32B/blob/main/config.json) has rms_norm_eps=1e-05 . We also overrided it, but it did not work:
- We also tested if tokenizer IDs matched between llama.cpp and normal Transformers courtesy of[@kalomaze](https://x.com/kalomaze/status/1897875332230779138) . They matched, so this was not the culprit.


We provide our experimental results inour docs .


🦥 Dynamic 4-bit Quants


We also uploaded[dynamic 4-bit](https://unsloth.ai/blog/dynamic-4bit) quants which increase accuracy vs naive 4-bit quantizations! We uploaded dynamic 4-bit quants to[here](https://huggingface.co/unsloth/QwQ-32B-unsloth-bnb-4bit) . We attached the QwQ quantization error plot analysis for both activation and weight quantization errors below:


Since[vLLM 0.7.3](https://github.com/vllm-project/vllm/releases/tag/v0.7.3) (2025 Feb 20), vLLM now supports loading Unsloth dynamic 4-bit quants!
