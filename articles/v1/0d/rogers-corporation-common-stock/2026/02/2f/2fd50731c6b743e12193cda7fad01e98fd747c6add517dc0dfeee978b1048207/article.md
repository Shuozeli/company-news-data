---
schema_version: "1.0.0"
document_id: "2fd50731c6b743e12193cda7fad01e98fd747c6add517dc0dfeee978b1048207"
company_key: "rogers-corporation-common-stock"
company: "Rogers Corporation Common Stock"
source_id: "rogers-corporation-common-stock-news-import-94efe698d8e2"
canonical_url: "https://www.rogerscorp.com/blog/2026/ul-9540-and-ul-9540a-how-fire-safety-standards-influence-material-selection-in-bess"
published_at: "2026-02-04T00:00:00+00:00"
first_seen_at: "2026-07-22T12:10:24.505358+00:00"
fetched_at: "2026-07-28T22:21:48.311987+00:00"
content_hash: "sha256:af4c05447f86bde4008b973ffabaf3538179e426ea12cd730aa5574def87cf2a"
---

# UL 9540 and UL 9540A: How Fire Safety Standards Influence Material Selection in BESS

# UL 9540 and UL 9540A: How Fire Safety Standards Influence Material Selection in BESS


*Published by Rogers Corporation [Elastomeric Material Solutions](https://www.rogerscorp.com/blog?qsBlogSection={951CE41D-76AB-4377-9060-5F437C222DD7})*


Battery energy storage systems, commonly referred to as BESS, play a critical role in renewable energy integration, backup power, and grid stability. As these systems scale in size and deployment, fire safety has become one of the most closely scrutinized aspects of their design.


Two standards now dominate BESS fire safety discussions: UL 9540 and UL 9540A.


UL 9540 is the system-level safety standard for energy storage systems, while UL 9540A provides a method for evaluating thermal runaway and fire propagation behavior. Neither standard certifies individual materials such as foams. However, because both evaluate system behavior, they strongly influence how engineers approach material selection inside BESS enclosures.


As a result, many design teams specify UL 94 V-0 rated foams for gaskets, gap fillers, and enclosure pads as a risk-reduction measure. While UL 9540A does not mandate material-level certifications, using self-extinguishing foams helps limit the potential contribution of materials during system-level fire testing.


### What UL 9540 and UL 9540A Evaluate


UL 9540 evaluates the overall safety of an energy storage system, including:


- Electrical design and protection
- Mechanical integrity
- Enclosure construction and containment
- Thermal management and spacing
- Alignment with codes such as NFPA 855 and the International Fire Code


A UL 9540 listing is typically required for BESS installations to be approved by Authorities Having Jurisdiction (AHJ).


UL 9540A evaluates thermal runaway and fire propagation behavior under controlled conditions. Testing:


- Begins at the cell level
- Scales through modules, units, and full installations
- Assesses whether a failure propagates through the system
- Measures heat release, gas generation, and fire behavior


These evaluations are conducted at accredited laboratories and are commonly reviewed during permitting. Failing UL 9540A testing can result in redesigns, retesting, and significant project delays.


*Image property of Rogers Corporation*


### Why Engineers Default to UL 94 V-0 Foams


Because UL 9540A testing is complex, time consuming, and costly to repeat, engineers often reduce risk earlier in the design process by making safe material choices for soft components.


Common reasons include:


- Self-extinguishing behavior that limits fire contribution
- Verified performance at specific thicknesses
- Reduced uncertainty during system-level fire evaluation


Even though UL 9540A does not explicitly require material certifications, many BESS programs treat UL 94 V-0 as a baseline expectation for foams and gaskets.


### Where UL 94 V-0 Foams Are Used in BESS


In BESS assemblies, soft materials are commonly required to manage tolerances, vibration, and sealing. Typical applications include:


- Module and cell gap fillers
- Enclosure gaskets and access panel seals
- Vibration and cushioning pads for power electronics
- Space-constrained designs requiring thin materials


In these applications,[UL 94 V-0 foams](https://www.rogerscorp.com/blog/2026/~/link.aspx?_id=1758E45B01B54711BE78DE8634FC159D&_z=z) can help balance mechanical performance with fire safety expectations.


### Comparing Flame Safety Standards in BESS


Standard Level Tested Focus Why It Matters to Engineers


UL 9540 Complete system Electrical, mechanical, enclosure, and code compliance Required listing for most BESS installations under NFPA 855 and the International Fire Code. Supports Authority Having Jurisdiction (AHJ) approval.


UL 9540A System under fire conditions (Cell → Module → Unit → Installation) Thermal runaway and fire propagation Evaluates whether failure propagates through the system. Results influence permitting, layout, and mitigation strategies.


UL 94 V-0 Material (foams, plastics, elastomers) Vertical flammability and self-extinguishing behavior Guides material selection. Using V-0 materials helps reduce risk during UL 9540A system-level fire testing.


### PORON® 40V0 Polyurethane in BESS Applications


PORON 40V0 polyurethane foam aligns with common BESS material requirements:


- UL 94 V-0 rated at 3 mm thickness
- Soft and compressible to absorb tolerances and reduce assembly stress
- Halogen-free and siloxane-free
- Proven long-term performance and reliability


In applications where extreme temperature stability or outdoor weathering resistance are not required, PORON 40V0 material can be a practical alternative to silicone-based materials. A[Rogers Sales Engineer](https://www.rogerscorp.com/SupportDetails?BusinessUnitId=%7B0075752E-56D3-4A95-BAC5-91E390374940%7D&SupportTypeId=%7BCD7C7B88-76A1-497F-9984-E0794AF70A94%7D) can help evaluate suitability for specific BESS designs.


### Key Takeaway


UL 9540 and UL 9540A define fire safety expectations for BESS at the system level. While these standards do not certify individual materials, reliable material selection helps reduce risk during system-level fire testing.


Using UL 94 V-0 foams for gaskets, gap fillers, and cushioning materials helps limit the contribution of soft components during UL 9540A evaluation and supports reliable long-term performance in modern BESS designs.


[Request a sample of PORON 40V0 foam](https://info.rogerscorp.com/RequestSample_PORON40V0) to evaluate it for your next energy storage project.


Published on Feb 04, 2026


## Submit Comment


[Return to Blog](https://www.rogerscorp.com/Blog)


**


### Related Blogs


[Beyond the Server Rack: Where Soft UL 94 V-0 Foams Solve Real Design Challenges](https://www.rogerscorp.com/blog/2026/beyond-the-server-rack-where-soft-ul-94-v-0-foams-solve-real-design-challenges)
Feb 04, 2026


[How PORON® 40V0 Foam Improves Low-Frequency Noise and Vibration Control in EVs and Data Centers](https://www.rogerscorp.com/blog/2025/how-poron-40v0-foam-improves-low-frequency-noise-and-vibration-control-in-evs-and-data-centers)
Nov 17, 2025


[Where UL 94 V-0 Matters Most: How to Prioritize Flame Ratings in Real-World Designs](https://www.rogerscorp.com/blog/2025/where-ul-94-v-0-matters-most-how-to-prioritize-flame-ratings-in-real-world-designs)
Oct 16, 2025


**


### Support


#### Elastomeric Material Solutions


Find local representatives for personalized support.
