...
document_id: "89c4e62096db799343c79987025a96fa273a69b6e8d983262561684b9b3c7dea"
...
canonical_url: "https://blog.cloudflare.com/meerkat-introduction/"
published_at: "2026-07-08T12:00:00+00:00"
first_seen_at: "2026-07-20T03:32:14.182345+00:00"
fetched_at: "2026-07-28T21:08:44.176891+00:00"
content_hash: "sha256:03aa5e2d99627cf308245fff366e5087a16193de8d0c68632d3cb63e5e2e4400"
---
...
Unfortunately, commonly deployed consensus algorithms like[Raft](https://raft.github.io/) suffer in wide-area networks like Cloudflare’s because they rely on *leaders* and *timeouts* . The *leader* is the only replica allowed to make writes, and if it fails due to a crash or network degradation, the system becomes unavailable until some other replica *times out* and a new leader is elected. And these timeout values are hard to configure in networks with unpredictable latencies.


We have experienced multiple incidents caused by unavailable leaders in consensus-driven systems.


And so, for the past year, Cloudflare’s Research[team](https://research.cloudflare.com/) has been building a new distributed consensus service called **Meerkat** powered by a consensus algorithm called[QuePaxa](https://bford.info/pub/os/quepaxa/quepaxa.pdf) , published in 2023 by Tennage & Băsescu et al. QuePaxa differs from Raft in that all replicas can perform writes at all times, and progress is never halted due to a timeout, which makes it well suited for Cloudflare’s network. We layer *applications* , like a transactional key-value store and leasing system, atop Meerkat’s consensus log. To our knowledge, this will be the first industrial deployment of QuePaxa at global scale.
...
Specifically, many algorithms that rely on authoritative leaders do not provide our desired availability guarantees, because they can become unavailable when a single machine experiences issues. Consider[Raft](https://raft.github.io/) , one of the most well-known and probably *the* most implemented consensus algorithm. Raft relies on an authoritative leader: the only replica in the cluster that can drive consensus. As a result, all writes get forwarded to the leader. This design choice helps make Raft “[understandable](https://raft.github.io/raft.pdf) ” and, coupled with[leases](https://dl.acm.org/doi/epdf/10.1145/3786663) , can make leader-served reads automatically linearizable (since they’re guaranteed to be up-to-date). But it also adds a single point of (temporary) failure.
...
Follow along on the Cloudflare Blog as Meerkat progresses, and check out more of our projects at[Cloudflare Research](https://research.cloudflare.com/) .
